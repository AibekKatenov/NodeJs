...
function sendRate(e){
    ...
    const blog = document.getElementById('comment-film').value

    if(activeStars.length > 0){
        axios.post('/api/rate', {rate: activeStars.length, text: comment_text, authorId: author, blogId: blog}).then(data => {
            if(data.data){
                location.reload()
            }
        })
